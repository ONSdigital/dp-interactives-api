...
func (api *API) UpdateInteractiveHandler(w http.ResponseWriter, req *http.Request) {
	...
	// 2.1 - fail if attempting to update the slug for a published model
	if *existing.Published && update.Interactive.Metadata != nil && existing.Metadata.HumanReadableSlug != update.Interactive.Metadata.HumanReadableSlug {
		http.Error(w, ErrCantUpdateSlug.Error(), http.StatusForbidden)
		log.Error(ctx, fmt.Sprintf("attempting to update slug for a published model existing (%s), update (%s)", existing.Metadata.HumanReadableSlug, update.Interactive.Metadata.HumanReadableSlug), ErrCantUpdateSlug)
		return
	}
	...
	updatedModel := &models.Interactive{
		ID:            id,
		Published:     update.Interactive.Published,
	...
	if update.Interactive.Metadata != nil {
		...
		// dont update title (is like a primary key)/url (is generated by the api from slug + resource-name/id etc)
