	...
	Convey("Given an environment with no environment variables set", t, func() {
		...
		Convey("When the config values are retrieved", func() {
			...
			Convey("Then the values should be set to the expected defaults", func() {
				...
				So(cfg.InteractivesWriteTopic, ShouldEqual, "interactives-api")
				So(cfg.GracefulShutdownTimeout, ShouldEqual, 5*time.Second)
				So(cfg.HealthCheckInterval, ShouldEqual, 30*time.Second)
				So(cfg.HealthCheckCriticalTimeout, ShouldEqual, 90*time.Second)
				So(cfg.MongoConfig.BindAddr, ShouldEqual, "localhost:27017")
				So(cfg.MongoConfig.Collection, ShouldEqual, "interactives")
				So(cfg.MongoConfig.Database, ShouldEqual, "interactives-api")
